...
### 报错：`AttributeError: 'Namespace' object has no attribute 'watch'`

这通常是旧脚本/旧安装与新参数不一致导致。请执行以下任一方式更新：
...
或直接使用仓库中的最新 `literature_manager.py` 文件运行。
...
```bash
python3 -m py_compile literature_manager.py test_literature_manager.py
python3 -m unittest -v test_literature_manager.py
```
